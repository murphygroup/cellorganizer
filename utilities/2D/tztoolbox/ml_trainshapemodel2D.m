function shapemodel = ml_trainshapemodel2D( shapes, param )
...
%         end
        ...
        for k=1:length(shapes)
            ...
            try
                if isempty(param.constknots{1})
                    medfeats(c,:) = [shapes{k}.length,knots1,coef1];
                else
                    medfeats(c,:) = [shapes{k}.length,coef1];
                end
                if isempty(param.constknots{2})
                    widthfeats(c,:) = [knots2,coef2];
                else
                    widthfeats(c,:) = coef2;
                end
                c = c+1;
        ...
        shapemodel.medaxis.stat = ml_estpdf2D(medfeats,struct('name','mvn'));
        shapemodel.width.stat = ml_estpdf2D(widthfeats,struct('name','mvn'));
...
%         shapemodel.constknots = param.constknots;
    ...
    case 'rds'
        if ~isfield(param,'ml_rdistpca')
            ...
            param.ml_rdistpca.anglestep = param.anglestep;
        end
        
        param.ml_rdistpca.anglestep = param.anglestep;
        shapemodel.name = 'rds';
        shapemodel = ml_initparam(shapemodel, ...
            ml_rdistpca2D(shapes,param.ml_rdistpca));
%         shapemodel = ml_initparam(shapemodel, ...
%             ml_rdistpca2D_modular(shapes,param.ml_rdistpca));
    otherwise
        shapemodel.name = 'unknown';
end
