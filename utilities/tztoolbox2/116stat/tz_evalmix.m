function [ress,mress]=tz_evalmix(combobj,weights,combcellidx, ...
    combclass,nfold,clstmeth,clustk,clstsda,classifmeth1,t1,option, ...
    loadpath,other,ntrial,classnum,cellnums,evalmeths,fitmeth, ...
    minalpha,estnum)
%TZ_EVALMIX Unknown.
...
%
%OVERVIEW:
%   Evaluate the goodness-of-fit of mixture models by crossvalidation
%PARAMETERS:
%   same as tz_testcv except
...
for kk=clustk
    ...
    for fold=1:nfold
        ...
        objnumfilename=['#$objnum' objtypefilename clusterfilename];
        if isempty(weights)
