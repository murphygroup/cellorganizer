...
%Author : Serena Abraham
    ...
    for i=1:length(az)
        if and(phi(i)<=max(spatial.anglesphi),phi(i)>=min(spatial.anglesphi(i)))
            new_az(end+1)=az(i);
            new_phi(end+1)=phi(i);
            new_r(end+1)=r(i);
        end
    ...
    y_b = mvksdensity(spatial.mappos_x(:,2:6),final_range2(:,2:6),'Bandwidth',0.3,'Kernel','normal');
    scatter3(x_1,y_1,z_1,'filled','CData',y_b);
