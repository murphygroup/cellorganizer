function primitive = createSBMLstruct3(model,currentmodel,modelclass,primitive,options)
%CREATESBMLSTRUCT3 This function returns a primitive structure for saving as SBML-Spatial
%
%Inputs: 
...
primitive.(modelclass).list = struct('type', {}, 'name', {}, 'position', {}, 'rotation', {}, 'scale', {}, 'rotationmatrix', {}, 'covariancematrix', {}, 'vol', {}, 'sa', {}, 'resolution', {});
...
%     primitive.(modelclass).list(i).scale = objsizevec(currobj,:).*model.resolution;
    ...
    primitive.(modelclass).list(i).resolution = [1,1,1];
...
if length(primitive.(modelclass).list) > 0
    ...
    primitive.(modelclass).totvol = 0;
    primitive.(modelclass).totsa = 0;
end
