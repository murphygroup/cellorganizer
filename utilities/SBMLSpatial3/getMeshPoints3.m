function FV = getMeshPoints3(image, savepath, downsampling, options)
%GETMESHPOINTS3 This function gets mesh points given an image
%
%Inputs: 
...
warning('off', 'CellOrganizer:getMeshPoints3');
...
warning('CellOrganizer:getMeshPoints3', 'Shouldn''t the threshold default to 0.5 if the image is binary?')
isopatch = 0.9;
%%%

disp('Entering mesh code.')
warning('CellOrganizer:getMeshPoints3', 'Converting image to single due to memory issues');
image = single(image);
...
warning('CellOrganizer:getMeshPoints3', 'Smoothing temporarily disabled due to memory issues, see utilities/3D/vesicles/3D/tp_imresize3d.m for example of estimating memory usage');
...
warning('CellOrganizer:getMeshPoints3', 'Downsampling temporarily using nearest due to memory issues');
