...
%    - surface2 can be the string 'self' to test for self-intersection
%
% OUTPUT:
...
selfIntersection = ischar(surface2) && strcmp(surface2, 'self');
if selfIntersection
  surface2 = surface1;
elseif isa(surface2, 'triangulation')
...
if selfIntersection
  ...
  % Allocation assumes a manifold mesh
  faceEdgesKeys1 = {};
  faceEdgesKeys2 = {};
  faceEdgesIndices = {};
  faceVerticesIndices1 = {};
  faceVerticesIndices2 = {};
  for i = 1:3
    indices1 = surface1.faces(:, i);
    indices2 = surface1.faces(:, mod(i, 3) + 1);
    indices = sort([indices1, indices2], 2);
    indices = (indices(:, 1) - 1) * nVert1 + indices(:, 2);
    faceEdgesKeys1{end+1} = (1:nFace1)';
    faceEdgesKeys2{end+1} = indices;
    faceVerticesIndices1{end+1} = (1:nFace1)';
    faceVerticesIndices2{end+1} = indices1;
    faceEdgesIndices{end+1} = repmat(i, nFace1, 1);
  end
  faceEdgesKeys1 = cell2mat(faceEdgesKeys1);
  faceEdgesKeys2 = cell2mat(faceEdgesKeys2);
  faceEdgesIndices = cell2mat(faceEdgesIndices);
  faceEdgeKeys = sparse(faceEdgesKeys2, faceEdgesKeys1, true, nVert1^2, nFace1, nFace1 * 6); % 0.4 MB
  faceEdgesIndices = sparse(faceEdgesKeys2, faceEdgesKeys1, faceEdgesIndices, nVert1^2, nFace1, nFace1 * 6); % 0.4 MB
  edgeKeysUnique = unique(faceEdgesKeys2);
  
  adjacentFaces = {};
  adjacentFaceEdgeIndicesFaces = {};
  adjacentFaceEdgeIndices = {};
  for i = edgeKeysUnique'
    edgeFaces = find(faceEdgeKeys(i, :));
    edgeIndices = faceEdgesIndices(i, edgeFaces);
    % Assumes a manifold mesh
    if length(edgeFaces) == 1
      aFRow = [edgeFaces(1), edgeFaces(1)];
      aFEIFRow = [];
      aFEIRow = [];
    elseif length(edgeFaces) == 2
      aFRow = [edgeFaces(1), edgeFaces(1); edgeFaces(1), edgeFaces(2); edgeFaces(2), edgeFaces(1); edgeFaces(2), edgeFaces(2)];
      aFEIFRow = [edgeFaces(1), edgeFaces(2); edgeFaces(2), edgeFaces(1)];
      aFEIRow = [edgeIndices(1); edgeIndices(2)];
    else
      error('Mesh is not manifold');
    end
    adjacentFaces{end+1, 1} = aFRow;
    adjacentFaceEdgeIndicesFaces{end+1, 1} = aFEIFRow;
    adjacentFaceEdgeIndices{end+1, 1} = aFEIRow;
  end
  adjacentFaces = cell2mat(adjacentFaces);
  adjacentFaces = unique(adjacentFaces, 'rows');
  faceEdgeAdjacency = sparse(adjacentFaces(:, 1), adjacentFaces(:, 2), true, nFace1, nFace1);
  adjacentFaceEdgeIndicesFaces = cell2mat(adjacentFaceEdgeIndicesFaces);
  adjacentFaceEdgeIndices = cell2mat(adjacentFaceEdgeIndices);
  faceEdgeAdjacencyIndices = sparse(adjacentFaceEdgeIndicesFaces(:, 1), adjacentFaceEdgeIndicesFaces(:, 2), adjacentFaceEdgeIndices, nFace1, nFace1);
  ...
  faceVertexIndices = {};
  for i = 1:3
    indices = surface1.faces(:, i);
    faceVerticesIndices1{end+1} = (1:nFace1)';
    faceVerticesIndices2{end+1} = indices;
    faceVertexIndices{end+1} = repmat(i, nFace1, 1);
  end
  faceVerticesIndices1 = cell2mat(faceVerticesIndices1);
  faceVerticesIndices2 = cell2mat(faceVerticesIndices2);
  faceVertexIndices = cell2mat(faceVertexIndices);
  faceVertices = sparse(faceVerticesIndices2, faceVerticesIndices1, true, nVert1, nFace1, nFace1 * 3);
  faceVertexIndices = sparse(faceVerticesIndices2, faceVerticesIndices1, faceVertexIndices, nVert1, nFace1, nFace1 * 3);
  adjacentFaces = {};
  adjacentFaceVertexIndicesFaces = {};
  adjacentFaceVertexIndices = {};
  for i = 1:nVert1
    vertexFaces = find(faceVertices(i, :));
    vertexFaceVertexIndices = faceVertexIndices(i, vertexFaces);
    [vertexFaces1, vertexFaces2] = ndgrid(vertexFaces);
    aFRow = [vertexFaces1(:), vertexFaces2(:)];
    adjacentFaces{end+1, 1} = aFRow;
    [vertexFaceVertexIndices1, vertexFaceVertexIndices2] = ndgrid(vertexFaceVertexIndices);
    aFVIFRow = aFRow;
    aFVIRow = vertexFaceVertexIndices1(:);
    adjacentFaceVertexIndicesFaces{end+1, 1} = aFVIFRow;
    adjacentFaceVertexIndices{end+1, 1} = aFVIRow;
  end
  adjacentFaces = cell2mat(adjacentFaces);
  adjacentFaces = unique(adjacentFaces, 'rows');
  faceVertexAdjacency = sparse(adjacentFaces(:, 1), adjacentFaces(:, 2), true, nFace1, nFace1);
  adjacentFaceVertexIndicesFaces = cell2mat(adjacentFaceVertexIndicesFaces);
  adjacentFaceVertexIndices = cell2mat(adjacentFaceVertexIndices);
  faceVertexAdjacencyIndices = sparse(adjacentFaceVertexIndicesFaces(:, 1), adjacentFaceVertexIndicesFaces(:, 2), adjacentFaceVertexIndices, nFace1, nFace1);
end
...
if selfIntersection
  intMatrix(speye(nFace1) > 0) = 0;   % face does not intersect itself
end
intSurface.vertices = [];
intSurface.faces    = [];
intSurface.edges    = [];

selfIntersectionShouldPlot = false;
% selfIntersectionShouldPlot = true
selfIntersectionPlotCount = 0;
warning('off', 'MATLAB:prnRenderer:opengl');
function selfIntersectionPlot(intMat)
  if ~selfIntersectionShouldPlot, return; end
  figure();
  set(gcf(), 'Units', 'inches', 'Position', [0, 0, 4, 4]);
  face_alpha = (any(intMat ~= 0, 2) | any(intMat ~= 0, 1)');
  fprintf('mean(face_alpha) = %e\n', mean(face_alpha));
  face_alpha = face_alpha * 0.5;
  edge_alpha = 0.5;
  face_color = 'r';
  patch_handle = patch(surface1, 'FaceColor', face_color, 'FaceAlpha', 'flat', 'FaceVertexAlphaData', face_alpha, 'AlphaDataMapping', 'none', 'EdgeAlpha', edge_alpha);
  axis image;
  
  selfIntersectionPlotCount = selfIntersectionPlotCount + 1;
  [st, sti] = dbstack();
  res = 300;
  renderer = '';
  % renderer = 'opengl';
  ...
  % renderer = 'painters';
  export_fig_args = {};
  export_fig_args{end + 1} = sprintf('%s_%04i_%04i.png', mfilename, selfIntersectionPlotCount, st(sti + 1).line);
  export_fig_args{end + 1} = sprintf('-r%i', res);
  if ~isempty(renderer)
    export_fig_args{end + 1} = sprintf('-%s', renderer);
  end
  export_fig(export_fig_args{:});
  fprintf('Wrote "%s"\n', export_fig_args{1});
  delete(patch_handle);
  close(gcf());
end
...
if debug && selfIntersection; selfIntersectionPlot(intMatrix); end
if(all(intMatrix==0)), return; end        % no intersections
intMatrix(du1==0 & du2==0 & du3==0) = -1; % coplanar with unknown overlap
if debug && selfIntersection; selfIntersectionPlot(intMatrix); end
if selfIntersection
  % A face sharing a vertex with another but with both vertices on the
  % same side of the plane of the second does not intersect it
  intMatrix(faceVertexAdjacency & du2.*du3 > 0) = 0;
  intMatrix(faceVertexAdjacency & du1.*du3 > 0) = 0;
  intMatrix(faceVertexAdjacency & du1.*du2 > 0) = 0;
  intMatrix(speye(nFace1) > 0) = 0;
  if debug && selfIntersection; selfIntersectionPlot(intMatrix); end
end
...
if selfIntersection
  ...
  if selfIntersection
    % Vertices should not intersect their faces
    dv(sub2ind(size(dv), repmat((1:nFace2)', 1, 3), surface2.faces))=inf;
  end
  ...
  if debug && selfIntersection
    assert(all(U1 == V1, 1:2));
    assert(all(U2 == V2, 1:2));
    assert(all(U3 == V3, 1:2));
    assert(all(N2 == N1, 1:2));
    assert(all(d2 == d1, 1:2));
    assert(all(dv1 == du1', 1:2));
    assert(all(dv2 == du2', 1:2));
    assert(all(dv3 == du3', 1:2));
  end
end
...
if selfIntersection
  % A face sharing a vertex with another but with both vertices on the
  % same side of the plane of the second does not intersect it
  intMatrix(faceVertexAdjacency & dv2.*dv3 > 0) = 0;
  intMatrix(faceVertexAdjacency & dv1.*dv3 > 0) = 0;
  intMatrix(faceVertexAdjacency & dv1.*dv2 > 0) = 0;
  intMatrix(speye(nFace1) > 0) = 0;
  if debug && selfIntersection; selfIntersectionPlot(intMatrix); end
end
if debug && selfIntersection
  assert(all(intMatrix == intMatrix', 1:2), 'intMatrix not symmetric')
end
...
tMsk = (intMatrix==-2);
if selfIntersection
  intMatrix(faceEdgeAdjacency & intMatrix == -2) = 0; % Faces sharing an edge and not coplanar do not intersect
  if debug && selfIntersection; selfIntersectionPlot(intMatrix); end
  tMsk = (intMatrix==-2);
end
n = nnz(tMsk);
  ...
  switch lower(algorithm)
    case 'moller'
      ...
      [intMatrix(tMsk), intSurface] = TriangleIntersection3D_Moller(...
        ...
        getIntersection, debug, selfIntersection);
    case 'rapid'
...
if selfIntersection
  tMsk = intMatrix==-1 & faceEdgeAdjacency;
  if nnz(tMsk)>0
    % Test for overlap between faces with a shared edge
    [face1, face2] = find(tMsk);
    faceEdgeAdjacencyIndices_full = full(faceEdgeAdjacencyIndices);
    face1EdgeIndex = faceEdgeAdjacencyIndices(sub2ind(size(tMsk), face1, face2));
    face2EdgeIndex = faceEdgeAdjacencyIndices(sub2ind(size(tMsk), face2, face1));
    face1EdgeVertexIndices = surface1.faces(sub2ind(size(surface1.faces), [face1, face1], mod(face1EdgeIndex + (-1:0), 3) + 1));
    face2EdgeVertexIndices = surface1.faces(sub2ind(size(surface1.faces), [face2, face2], mod(face2EdgeIndex + (-1:0), 3) + 1));
    face1EdgeOtherVertexIndex = surface1.faces(sub2ind(size(surface1.faces), face1, mod(face1EdgeIndex + 1, 3) + 1));
    face2EdgeOtherVertexIndex = surface1.faces(sub2ind(size(surface1.faces), face2, mod(face2EdgeIndex + 1, 3) + 1));
    face1EdgeVertex1 = surface1.vertices(face1EdgeVertexIndices(:, 1), :);
    face1EdgeVertex2 = surface1.vertices(face1EdgeVertexIndices(:, 2), :);
    face1EdgeOtherVertex = surface1.vertices(face1EdgeOtherVertexIndex, :);
    face2EdgeOtherVertex = surface1.vertices(face2EdgeOtherVertexIndex, :);
    EV1 = face1EdgeVertex1;
    EV2 = face1EdgeVertex2;
    OV1 = face1EdgeOtherVertex;
    OV2 = face2EdgeOtherVertex;
    % Edge vector
    EV = EV2-EV1;
    % Other vertex 1 minus edge vertex 1
    OE1 = OV1-EV1;
    % Other vertex 2 minus edge vertex 1
    OE2 = OV2-EV1;
    lenfunc = @(x) sqrt(dot_prod(x, x));
    normfunc = @(x) x ./ repmat(lenfunc(x), 1, 3);
    projnormfunc = @(x, y) y .* dot_prod(x, y);
    rejnormfunc = @(x, y) x - y .* dot_prod(x, y);
    % projfunc = @(x, y) projnormfunc(x, normfunc(y));
    EVn = normfunc(EV);
    % Rejection of OE1 from edge vector
    OE1r = rejnormfunc(OE1, EVn);
    % Rejection of OE2 from edge vector
    OE2r = rejnormfunc(OE2, EVn);
    % Dot product of rejections will be 1 if the other vertices are on
    % the same side and the triangles overlap
    tMskInt = dot_prod(OE1r, OE2r) > 0;
    intMatrix(tMsk) = 0;
    intMatrix(sparse(face1, face2, tMskInt, size(intMatrix, 1), size(intMatrix, 2))) = 1;
  end
  if debug && selfIntersection; selfIntersectionPlot(intMatrix); end
  tMsk = intMatrix==-1 & faceVertexAdjacency;
  if nnz(tMsk)>0
    % Test for overlap between faces with a shared vertex
    [face1, face2] = find(tMsk);
    face1VertexFaceIndex = faceVertexAdjacencyIndices(face1);
    face2VertexFaceIndex = faceVertexAdjacencyIndices(face2);
    face1VertexIndex = surface1.faces(face1, face1VertexFaceIndex);
    face1OtherVertexIndices = surface1.faces(face1, mod(face1VertexFaceIndex + (0:1), 3) + 1);
    face2OtherVertexIndices = surface1.faces(face2, mod(face2VertexFaceIndex + (0:1), 3) + 1);
    face1Vertex = surface1.vertices(face1VertexIndex(:, 1), :);
    face1Edge1OtherVertex = surface1.vertices(face1OtherVertexIndices(:, 1), :);
    face1Edge2OtherVertex = surface1.vertices(face1OtherVertexIndices(:, 2), :);
    face2Edge1OtherVertex = surface1.vertices(face2OtherVertexIndices(:, 1), :);
    face2Edge2OtherVertex = surface1.vertices(face2OtherVertexIndices(:, 2), :);
    SV = face1Vertex;
    F1E1V = face1Edge1OtherVertex;
    F1E2V = face1Edge2OtherVertex;
    F2E1V = face2Edge1OtherVertex;
    F2E2V = face2Edge2OtherVertex;
    % Is either edge of one face between the two edges of the other face?
    tMskInt = dot_prod(cross_prod(F1E1V-SV,F2E1V-SV), cross_prod(F1E1V-SV,F2E2V-SV)) < 0;
    tMskInt = tMskInt | dot_prod(cross_prod(F1E2V-SV,F2E1V-SV), cross_prod(F1E2V-SV,F2E2V-SV)) < 0;
    intMatrix(tMsk) = 0;
    intMatrix(sparse(face1, face2, tMskInt, size(intMatrix, 1), size(intMatrix, 2))) = 1;
  end
  if debug && selfIntersection; selfIntersectionPlot(intMatrix); end
...
%      are performed.
%   selfIntersection - are we finding the self-intersection of a single surface?
%
% OUTPUT:
...
B = U1-U2;
C = U1-V1;
identical = all((V1 == U1 & V2 == U2) | (V1 == U2 & V2 == U1), 2);
intersect = false(size(A, 1), 1);
X = nan(size(A));

% Process non-identical edges
rows = ~identical;
...
det3 = @(a,b) ... % determinant of a matrix with columns: [a, b, 1]
  ...
  a(:,2).*b(:,3)-a(:,2).*b(:,1) + ...
  a(:,3).*b(:,1)-a(:,1).*b(:,3);
f=det3(A(rows,:),B(rows,:)); % https://en.wikipedia.org/wiki/Cramer%27s_rule#Explicit_formulas_for_small_systems
t=det3(C(rows,:),B(rows,:))./f; % use Cramer's rule
s=det3(A(rows,:),C(rows,:))./f;
intersect(rows) = (t>=0 & t<=1 & s>=0 & s<=1);
X(rows,:) = V1(rows,:) + bsxfun(@times,A(rows,:),t);

% Process identical edges
rows = identical;
% intersect(rows) = false; % Identical edges are not intersections
% Identical edges are intersections if adjacent faces are not passed to this function
intersect(rows) = true;
X(rows,:) = (V1(rows,:) + V2(rows,:))./2;
end % function
