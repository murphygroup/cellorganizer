function [segdna,segcell] = seg_cell_and_nucleus2dproj( dna_image_file, ...
    cell_image_file, ...
    prot_image_file, ...
    mask_image_file, ...
    downsample, ...
...
%D. Sullivan 5/5/14 - why do we care about improt for seg_cell_and_nuc if
%not using the nuc finding. Should check this only when we need it. 
try
    improt = ml_readimage( prot_image_file );
catch err
    disp( [ 'Unable to load image: ' prot_image_file ] );
    getReport( err, 'extended' )
    segdna = []; segcell = []; return
end
...
if length(unique(dnaim3)) > 2
    ...
    psf_filename = '';
    [segdna, ~, ~, ~, zflip] = preprocess(dnaim3, dnafile, psf_filename, downsample, false, 0.95, 0.05, display, segcell, param );
    
    %D. Sullivan 5/5/14 - temporary for making 2D proj of cell.
     %D. Sullivan 5/5/14 - temporary for making 2D proj of cell.
        segcell = squeeze(sum(segcell,3))>0;
        top_slice = 1;
        bot_slice = 1;
        segdna = squeeze(sum(segdna,3))>0;
        top_slice = 1;
        bot_slice = 1;
        sumslices = sum(segcell(:));
