...
for posval=1:length(patchlist.F),
    ...
    if isfield(data,'mask')
        if ~isempty(data.mask)
            patchfilter = [zeros(grownpatchsize(1)-1,grownpatchsize(2)*2-1); ...
                    zeros(grownpatchsize(1),grownpatchsize(2)-1) ...
                    ones(grownpatchsize)];
            errormask = imfilter(double(data.mask), ...
                patchfilter,'same');
            %-0.5 was added for dealing with rounding error
            errorimg(errormask<prod(grownpatchsize)-0.5) = inf;
        end
    end
    ...
    [S,bestmatch_mse,bestmatch_ssd,numpixels] =...
        tz_errorsurface(shiftT,grownpatchsize(1),grownpatchsize(2),J,I);
    ...
    if (patch_overlap_error > data.patch_delta_max & ...
        ...
        figure(1); imshow(out,'InitialMagnification',100)
    end
