...
%     text(sum(Real_x_solve)/length(Real_x_solve)-20,sum(Real_y_solve)/length(Real_y_solve)-20,num2str(cnt),'FontSize',18);
    ...
    if size(JiaoDian_Point, 1) > 2 && size(JiaoDian_Point_shang, 1) == 2
        dists = pdist2(JiaoDian_Point, JiaoDian_Point_shang);
        [~, min_inds] = min(dists);
        if min_inds(1) ~= min_inds(2)
            JiaoDian_Point = [JiaoDian_Point(min_inds(1), :); JiaoDian_Point(min_inds(2), :)];
        else
            JiaoDian_Point = [];
        end
    elseif size(JiaoDian_Point, 1) > 2
        JiaoDian_Point = [];
    end    
else
    JiaoDian_Point=[];
