function [ZhixinX,ZhixinY,lunkuo_of_ImgInput1,Img,Relative, Position]=SmallCellTracking2(ImgInput2,ZhixinX,ZhixinY,width,Img,Processdir,j, param)
% In the next frame, a square region with a side length of 60 is extracted from the center of the above frame

if param.verbose
    Processdir_small=strcat(Processdir,'/Small/',num2str(j));
    if ~exist(Processdir_small, 'dir')
        mkdir(Processdir_small);
    end
end
Dimg=double(Img);
Prearea=sum(sum(Dimg));
...
se = strel('disk',1);
BW = imclose(BW,se);
% figure(100),subplot(121),imshow(BW)
if param.verbose
    imwrite(BW,strcat(Processdir_small,'/','01Otsu_Imclose.png'),'png');
end
bb=imfill(BW,'holes');
[L,num] = bwlabel(bb,8);
...
%     figure(101),imshow(img44);
    ...
    % if there are more than 1 region with maximum area, chose the one
    % whose centroid is closest to the center of the image. 
	if length(ind) > 1
		Centroids = cat(1, stats.Centroid);
        Maxima_centroids = Centroids(ind, :);
        [~, closest_ind] = min(pdist2(Maxima_centroids, ([size(L2, 2), size(L2, 1)] + 1) ./ 2));
        ind = ind(closest_ind);
    end    
