function [JiaoDian_Point]=DrawPointsInLoop(j,target_lunkuo,Circle_LunKuo,Center,CircleR,JiaoDian_Point_shang,Superposition)
...
[a,b,q,p,p2,X_center,Y_center,xmin,xmax,ymin,ymax,F]=Ellipse_Fitting([target_lunkuo(:,1),target_lunkuo(:,2)]);
% hold on
...
[x_solve,y_solve]=vpasolve(p(1)*x^2+p(2)*x*y+p(3)*y^2+p(4)*x+p(5)*y+p(6)==0, (x-x0)^2+(y-y0)^2== R^2, x, y);
...
%     text(sum(Real_x_solve)/length(Real_x_solve)-20,sum(Real_y_solve)/length(Real_y_solve)-20,num2str(cnt),'FontSize',18);
    ...
    % big cell and small cell overlap area as a percentage of small cell area
