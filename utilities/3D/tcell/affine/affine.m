...
%
% Changes copyright 2012 Taraz Buck (tebuck).
% Modified 2012-04-30 tebuck
% Added option to use same image bounds.
function [new_img, new_M] = affine(old_img, old_M, new_elem_size, verbose, bg, method, bounds)
   ...
   if isempty(bounds) || strcmpi(bounds, 'loose')
   ...
   elseif strcmpi(bounds, 'crop')
     new_dim = round(old_dim .* new_elem_size);
   else
     % Not sure if this is correct:
     new_M(1:3,4) = bounds(:, 1) .* new_elem_size;
     new_dim = bounds(:, 2) - bounds(:, 1);
   end
