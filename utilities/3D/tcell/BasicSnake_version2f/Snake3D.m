...
%
% 2012-09-02 tebuck: modifying plots for my microscopic data.
% Should extend this so that it can use maximum and root mean squared movement controls, which will give step sizes meaning independent of image scale. Also non-Euler integration?
% 2013-01-15 tebuck: permitting anisotropy in gradients and energies and such would be nice...
...
defaultoptions.voxel_scales = [1, 1, 1];
...
Eext = ExternalForceImage3D(I,Options.Wline, Options.Wedge,Options.Sigma1, Options.voxel_scales);

% Make the external force (flow) field.
Fx=ImageDerivatives3D(Eext,Options.Sigma2,'x', Options.voxel_scales);
Fy=ImageDerivatives3D(Eext,Options.Sigma2,'y', Options.voxel_scales);
Fz=ImageDerivatives3D(Eext,Options.Sigma2,'z', Options.voxel_scales);
...
Fext=GVFOptimizeImageForces3D(Fext, Options.Mu, Options.GIterations, Options.Sigma3, Options.voxel_scales);
if Options.Verbose
  fprintf('GVF done\n')
  for index = 1:5
    beep, pause(.2)
...
if(Options.Verbose)
  ...
  force_range = max(abs(Fext(:))) * 2;
  
  % Use reshape_contrast instead:
  number_reshape_rows = 6;
  % subplot(2, 1, 1)
  force_image_to_show = cat(3, reshape_2d(Fext(:, :, :, 1) / force_range + .5, number_reshape_rows), reshape_2d(Fext(:, :, :, 2) / force_range + .5, number_reshape_rows), reshape_2d(Fext(:, :, :, 3) / force_range + .5, number_reshape_rows));
  % imshow(force_image_to_show)
  % subplot(2, 1, 2)
  rasterization_mesh = FV;
  rasterization_mesh.vertices = rasterization_mesh.vertices(:, [2, 1, 3]);
  segmentation_image = voxelise(1:size(I, 2), 1:size(I, 1), 1:size(I, 3), rasterization_mesh);
  segmentation_image = permute(segmentation_image, [2, 1, 3]);
  segmentation_image_perimeter = bwperim(segmentation_image);
  segmentation_image_perimeter = ~segmentation_image_perimeter;
  image_to_show = force_image_to_show .* repmat(reshape_2d(segmentation_image_perimeter, number_reshape_rows), [1, 1, 3]);
  imshow_handle = imshow(image_to_show);
  % imshow_handle = imshow(image_to_show, 'border', 'tight');
...
for i=1:Options.Iterations
    ...
    % if(Options.Verbose)
    if(Options.Verbose) && mod(i, ceil(Options.Iterations / 10)) == 0
        % if(ishandle(ph));
        if(ishandle(gca));
            % delete(gca)
            force_range = max(abs(Fext(:))) * 2;
            ...
            segmentation_image_perimeter = ~segmentation_image_perimeter;
            image_to_show = force_image_to_show .* repmat(reshape_2d(segmentation_image_perimeter, number_reshape_rows), [1, 1, 3]);
            % imshow(image_to_show, 'border', 'tight')
            set(imshow_handle, 'CData', image_to_show)
...
if isfield(Options, 'save_last_frame') && Options.save_last_frame
  number_reshape_rows = 6;
  % subplot(2, 1, 1)
  force_image_to_show = cat(3, reshape_2d(Fext(:, :, :, 1) / force_range + .5, number_reshape_rows), reshape_2d(Fext(:, :, :, 2) / force_range + .5, number_reshape_rows), reshape_2d(Fext(:, :, :, 3) / force_range + .5, number_reshape_rows));
  rasterization_mesh = FV;
  ...
  segmentation_image_perimeter = bwperim(segmentation_image);
  segmentation_image_perimeter = ~segmentation_image_perimeter;
  % segmentation_image_perimeter = ~segmentation_image_perimeter - imdilate(segmentation_image_perimeter, true(3, 3, 3));
  % segmentation_image_perimeter = segmentation_image_perimeter | ~imdilate(segmentation_image_perimeter, true(3, 3, 3));
  % image_to_show = reshape_2d(image_to_show, number_reshape_rows);
  image_to_show = force_image_to_show .* repmat(reshape_2d(segmentation_image_perimeter, number_reshape_rows), [1, 1, 3]);
  mkdir(mfilename)
  imwrite(image_to_show, [mfilename, filesep, datestr(now(), 'yyyymmddHHMMSSFFF'), '.png'])
end
