function [result_var] = update_running_var(given_preceding_mean, given_preceding_var, given_parameters, given_index)
  % Update given_preceding_mean computed from (given_index - 1) data with given_parameters. Initial call should be with given_preceding_mean = zeros, given_index = 1.
  %
  % References:
  ...
  % 2015-07-12 xruan: Created
  % whos
  result_var = given_preceding_var * given_index ./ ( given_index + 1) ...
      + (given_parameters - given_preceding_mean) .^ 2 .* given_index ./ (given_index + 1 ) .^ 2;
