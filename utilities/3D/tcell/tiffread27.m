function stack = tiffread27(filename, indices, use_waitbar, verbose)
...
%
% Modified 2014-01-25 tebuck: Added verbose argument to control printing errors/warnings to screen via fprintf.
...
if ~exist('indices', 'var') || isempty(indices);  indices = 1:10000; end
if ~exist('use_waitbar', 'var');  use_waitbar = true; end
...
while  TIF.img_pos ~= 0 
    ...
    for i = 1:num_entries
        ...
        switch TIF.entry_tag
            ...
            case 34412       % Zeiss LSM data
                LSM_info           = entry.val;
            otherwise
                if verbose, fprintf( 'Ignored TIFF entry with tag %i (cnt %i)\n', TIF.entry_tag, entry.cnt); end
        end
    ...
    if isfield( TIF, 'MM_stack' )
        ...
        if numel(indices) > 1 && use_waitbar
        ...
        if read_img
            ...
            try
                ...
                if verbose, fprintf('Tiffread skipped dissimilar image %i\n', img_read+img_skip); end
                img_skip = img_skip + 1;
             end
...
if isfield(TIF, 'MM_stack') && isfield(IMG, 'info') && ~isempty(IMG.info)
    MM = parseMetamorphInfo(IMG.info, TIF.MM_stackCnt); % we dont have to use the Metamorph info
%     for i = 1:numel(stack)
%         stack(i).MM = MM(i);
%     end
end
...
%             case 'Background subtracted (Offset'
%                 mm(k).Background = 0;
%             otherwise
%                 field  = regexprep(tok, ' ', '');
%                 if strcmp(val, 'Off')
