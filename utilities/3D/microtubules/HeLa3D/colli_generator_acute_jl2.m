function [G,tcheck,imXYZ,mtXYZ,randlengths] = colli_generator_acute_jl2(n,mu_len,sigma_len,colli_min_number,Dboth,imgcent_coordinate,XYZres,centroradius,stepsize,rand_seed,cellnum,subfolder,batchno,colli_thre,angle_thre,checkback,nosave,nomcheck)
...
if nosave==0
save(['outputs_' num2str(subfolder) '/images/cell_' num2str(cellnum) '/batch_' num2str(batchno) '/new_n_' num2str(n) '/new_sim_n_' num2str(n) '_mulen_' num2str(mu_len) '_siglen_' num2str(sigma_len) '_colli_' num2str(colli_min_number) '-tcheck.mat'], 'tcheck', 'imgcent_coordinate', 'imXYZ', 'mtXYZ', 'G', 'n','randlengths','colli_thre','angle_thre','checkback');  %%
end
...
if n == 0
	...
	randlengths = 0;
		...
		if nosave==0
		save(['outputs_' num2str(subfolder) '/images/cell_' num2str(cellnum) '/batch_' num2str(batchno) '/new_n_' num2str(n) '/new_sim_n_' num2str(n) '_mulen_' num2str(mu_len) '_siglen_' num2str(sigma_len) '_colli_' num2str(colli_min_number) '.mat'], 'tcheck', 'imgcent_coordinate', 'imXYZ', 'mtXYZ', 'G', 'n','randlengths','colli_thre','angle_thre','checkback');  %%
		end
	return
end
