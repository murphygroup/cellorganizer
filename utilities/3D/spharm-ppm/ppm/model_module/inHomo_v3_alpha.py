...
regex = re.compile(r'sl3-AR3-AR4-NeuN-MBP-VDAC-40x.*.nd2')
...
for qind in xrange(0,len(quads)):
    ...
    try:
        ...
        beta = FitModel(q,temp_folder_name,img_names[qind],covar=factor_matrix)
        betas.append(beta)
    except:
        failed.append(qind)
        print 'failed',qind
...
with open('beta_file.txt','w') as f:
    for val in betas:
        for b in val:
            f.write(str(b)+' ')
        f.write('\n')
...
with open('res_file.txt','a') as f:
        f.write('\n')
        f.write('mean: '+str(avg_ll)+'\n')
        f.write('var: '+str(var_ll)+'\n')