...
# def FitModel( quad,temp_folder_name,img_name,covar):
	...
	for i in xrange(0,len(quads)):
		...
		xmax,ymax,zmax=find_xmax_ymax_zmax(temp_folder_name,img_name)
		...
		print 'generating factor matrix for dummy points...'
...
def inhomoLogLike(pat,coefs,temp_folder_name,img_name):
	...
	norm = np.mean(trends)
	# logdens = sum(np.log10(trendValue(coefs,temp_folder_name,img_name)/norm))
	# return (logdens/pat.npoints + np.log10(.01))
	logdens = sum(np.log(trendValue(coefs,temp_folder_name,img_name)/norm))
	return (logdens/pat.npoints + np.log(.01))
...
def points2factor(points,temp_folder_name,img_name):
	...
	for fi in xrange(0,len(factor_channel_num)): ###3 be param
		mat_contents = sio.loadmat(temp_folder_name+'dist_trans_matrix/'+img_name+'__dist_trans_matrix_'+str(factor_channel_num[fi])+'.mat')
...
def find_xmax_ymax_zmax(temp_folder_name,img_name):
	factor_channel_num = []
	with open('factor_channel_num.txt') as f:
		for line in f:
			factor_channel_num=[int(x) for x in line.split(',')]
	mat_contents = sio.loadmat(temp_folder_name+'dist_trans_matrix/'+img_name+'__dist_trans_matrix_'+str(factor_channel_num[0])+'.mat')
	dist_matrix=mat_contents['m']
	# factor_col[ci]=dist_matrix[x][y][z]	
