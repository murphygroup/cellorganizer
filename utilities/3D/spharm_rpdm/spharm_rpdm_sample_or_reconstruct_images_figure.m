...
switch options.shape_evolution
   case 'random'      
       ...
       'pair_method', 'random',...  % 'random' or 'given'
       'num_steps', 10  ...
    ));          

  case 'first two shapes'
     options = ml_initparam(options, struct( ...
    'pair_method', 'given', ...                % 'random' or 'given'
    'pair_inds', [1, 2], ...                    & If pair_method is 'given', must provide a pair of inds
    'num_steps', 10  ...
    ));
  case 'first and last shape'
      options = ml_initparam(options, struct( ...
    'pair_method', 'given', ...                % 'random' or 'given'
    'pair_inds', [1, size(train_score,1)], ...                    & If pair_method is 'given', must provide a pair of inds
    'num_steps', 10  ...
    ));
  otherwise
...
switch options.pair_method
    ...
    case 'given'
        ...
        if any(options.pair_inds > size(train_score, 1)) || any(options.pair_inds <= 0)
            warning('The given pair %d and %d are not valid, skip it!', options.pair_inds(1), options.pair_inds(2));
            return;
        end
end
