...
function [sph_verts, poles, dateline] = initParamCALD(vertices, faces, filename, confs)
...
fprintf('%s: %+4d = %d (%d) vertices * 2 - %d faces; %d edges\n', name, ...
...
% [poles, direct2nd] = determine_poles_1(vertices); % north 1 south 2
[poles, direct2nd] = determine_poles_2(vertices); % north 1 south 2
...
% instead of using PCa to determine north pole and south pole, we use
% projected image to xy plane and find the major axis as direct-1st, direct-2nd 
% is normal to it in xy plane and direct-3rd is (0, 0, 1), that is
% z-axis

function [poles, direc] = determine_poles_1(vertices)

pca_dim = 3;
% just use projected image in xy plane
[pca_ps, pca_b, var_amt, latent] = do_pca(vertices(:, 1:2),pca_dim);
...
direc1st = pca_b(:, 1);
min_pair = zeros(n, 3);
min_pair_distances = zeros(n, 1);
for i = 1 : n
    cur_vert = vertices(min_i(i), :);
    cur_layer = vertices(vertices(:, 3) == cur_vert(3), :);
    [cur_pair, cur_distance] = intersection_curve_line(cur_vert, direc1st, cur_layer);
    min_pair(i, :) = cur_pair;
    min_pair_distances(i) = cur_distance;
end

max_pair = zeros(m, 3);
max_pair_distances = zeros(m, 1);
for i = 1 : m
    cur_vert = vertices(max_i(i), :);
    cur_layer = vertices(vertices(:, 3) == cur_vert(3), :);
    [cur_pair, cur_distance] = intersection_curve_line(cur_vert, direc1st, cur_layer);
    max_pair(i, :) = cur_pair;
    max_pair_distances(i) = cur_distance;    
end

% pick the pair with closest distance to the centroid. 
directions = [min_pair; max_pair] - vertices([min_i; max_i], :);
directions = directions ./ sqrt(sum(directions .^ 2, 2));
centroid = mean(vertices);
centroid_distances = abs(sum((centroid - vertices([min_i; max_i], :)) .* directions, 2));
[~, min_ind] = min(centroid_distances);

all_pairs = [min_pair; max_pair];
all_pair_inds = zeros(size(all_pairs, 1), 1);
for i = 1 : size(all_pairs, 1)
    all_pair_inds(i) = find(sum(abs(vertices - all_pairs(i, :)), 2) == 0);  
end
all_minmax_i = [min_i; max_i];

pole_1 = all_minmax_i(min_ind);
pole_2 = all_pair_inds(min_ind);

if vertices(pole_1,1) > vertices(pole_2, 1)
    poles = [pole_2 pole_1];
else
    poles = [pole_1 pole_2];
end
...
function [cur_point, cur_distance] = intersection_curve_line(point_1, direction, curve)
% the curve is a close curve, so just find the point very close to the line

direction = direction ./ norm(direction);
distances = (curve(:, 1:2) - point_1(:, 1:2)) * [direction(2), -direction(1)]';
[distances_1, sorted_inds] = sort(abs(distances));

distances_point = pdist2(point_1, curve);
thrsh = prctile(distances_point, 90);

if false
    figure, plot(curve(:, 1), curve(:, 2), '.');
    hold on, plot(point_1(:, 1), point_1(:, 2), 'r*');
end
for i = 1 : numel(distances_1)
    if i == 1
        continue;
    end
    cur_ind = sorted_inds(i);
    cur_point = curve(cur_ind, :);
    cur_distance = distances_point(cur_ind);
    if false
        hold on, plot(cur_point(:, 1), cur_point(:, 2), 'bo');
    end
    cur_direction = cur_point - point_1;
    cur_direction = cur_direction ./ norm(cur_direction);
    cur_cos_angle = abs(cur_direction(1:2) * direction);
    if cur_distance > thrsh && cur_cos_angle > cos(pi / 24)
        break;
    end
end
...
% use a different way to decide north and south pole, that is, for each z,
% find the xy plane and pick the two points with largest distance, and use
% the two points with largest distance amonge all layers. 

function [poles, direc] = determine_poles_2(vertices)

z_values = unique(vertices(:, 3));

best_distance = 0;
for i = 1 : numel(z_values)
    cur_layer = vertices(vertices(:, 3) == z_values(i), :);
    [cur_distances, inds] = pdist2(cur_layer, cur_layer, 'Euclidean',  'Largest', 1);
    [max_dist, max_dist_ind] = max(cur_distances);
    if max_dist > best_distance
        best_distance = max_dist;
        pair_inds = [max_dist_ind, inds(max_dist_ind)];
        best_coords = [cur_layer(pair_inds(1), :); cur_layer(pair_inds(2), :)];
    end    
end

all_pair_inds = zeros(size(best_coords, 1), 1);
for i = 1 : size(best_coords, 1)
    all_pair_inds(i) = find(sum(abs(vertices - best_coords(i, :)), 2) == 0);  
end

pole_1 = all_pair_inds(1);
pole_2 = all_pair_inds(2);
