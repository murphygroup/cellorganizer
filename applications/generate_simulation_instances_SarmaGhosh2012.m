...
reaction_network_files{end}{end+1} = '../../data/SarmaGhosh2012ForCO.vcml';

reaction_network_files{end}{end+1} = '../../data/lotvol5r3surfcatal.*.mdl';
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
disp( mfilename );
disp( 'The estimated running time is about 20 minutes. Please wait.' );

% Combine SPHARM framework with vesicle models
combined_models = 'combined_model.mat';
chunk_filename = combined_models;
[can_start, final_name, final_exists, temp_name] = chunk_start(chunk_filename, '.mat', true);
if can_start
...
for i = 1:length(reaction_network_files)
    ...
    for j = 1:length(base_seed_reaction_network_files)
        ...
        seeds = reaction_network_file_base_seed + (0:n_images_to_synthesize - 1);

        seeds_finished = false(size(seeds));
        ...
        if strcmpi(reaction_network_file_ext, '.vcml') && all(seeds_finished)
